...
def aplicar_habilidad(player_id, accion):
    ...
    if accion == "danio_pincho":
        # Daño autoritativo del servidor para que no se revierta al sincronizar HP.
        if datos[2] == "practica":
            _daniar_jugador(player_id, 3)
        return

    if accion == "sanacion":
        # El sanador sana a todos los cercanos
        datos[7] = min(datos[8], datos[7] + 30)
        px, py = datos[0], datos[1]
        for otro_pid, otro_datos in players.items():
            if otro_pid != player_id and otro_datos[2] == "subterraneo" and not otro_datos[9]:
                dist = math.hypot(px - otro_datos[0], py - otro_datos[1])
                if dist < 150:  # Alcance de curación
                    otro_datos[7] = min(otro_datos[8], otro_datos[7] + 30)
        return
    ...
    elif accion == "hechizo_fuego" and dist < 360:  # Doble de alcance
        raton["hp"] = max(0, raton["hp"] - 60)  # Doble de daño
